...
WITH source_data AS (
  ...
  SELECT
    ...
    @generate_surrogate_key__sha_256(source, id)::BINARY AS supply_hk,
    @generate_surrogate_key__sha_256(source, id, valid_from)::BINARY AS supply_pit_hk,
    @generate_surrogate_key__sha_256(source, sku)::BINARY AS product_hk,
    @generate_surrogate_key__sha_256(source, id, sku)::BINARY AS supply_hk__product_hk,
    id AS supply_bk,
    sku AS product_bk,
    *
  FROM casted_data
